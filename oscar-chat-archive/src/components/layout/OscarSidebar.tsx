...
export function OscarSidebar() {
  ...
  const currentUser = useAuthenticatedQuery(api.users.current); // TODO: Fix with NextAuth integration
  
  // Fetch files based on authentication status
  const authenticatedFiles = useAuthenticatedQuery(api.files.list);
  const publicFiles = useQuery(api.files.listPublic, {});
