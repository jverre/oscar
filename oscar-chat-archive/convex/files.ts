...
export const create = mutation({
  args: {name: v.string(),
    ...
    metadata: v.optional(v.any()), userId: v.id("users")},
  handler: async (ctx, args) => {
...
export const getByFileName = query({
  args: {fileName: v.string(), userId: v.id("users")},
  handler: async (ctx, args) => {
    const userId = args.userId;
...
export const getByName = query({
  args: {organizationId: v.id("organizations"),
    name: v.string(), userId: v.optional(v.id("users"))},
  handler: async (ctx, args) => {
    const userId = args.userId;
...
export const updateName = mutation({
  args: {fileId: v.id("files"),
    name: v.string(), userId: v.id("users")},
  handler: async (ctx, args) => {
    const userId = args.userId;
...
export const updateVisibility = mutation({
  args: {fileId: v.id("files"),
    visibility: v.union(v.literal("public"), v.literal("private")), userId: v.id("users")},
  handler: async (ctx, args) => {
    const userId = args.userId;
...
export const getByNameOrPublic = query({
  args: {fileName: v.string(), userId: v.optional(v.id("users"))},
  handler: async (ctx, args) => {
    const userId = args.userId;
...
export const getFileWithOrgData = query({
  args: {fileId: v.id("files"), userId: v.optional(v.id("users"))},
  handler: async (ctx, args) => {
    const userId = args.userId;
    const file = await ctx.db.get(args.fileId);
