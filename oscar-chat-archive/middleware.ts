...
export default auth(async (request: NextRequest) => {
  ...
  if (subdomain) {
    // Allow access to auth routes on subdomains (for cross-subdomain auth)
    if (url.pathname.startsWith('/auth/')) {
      return NextResponse.next();
    }
    
    // With cross-subdomain cookies, we should allow access to subdomains
    // The client-side auth check will handle redirects if needed
    // Only redirect to signin if explicitly accessing protected routes
...
function getSubdomain(hostname: string): string | null {
  // Handle app.local development
  if (hostname.includes("app.local")) {
    // Extract subdomain from app.local:3000 or subdomain.app.local:3000
  ...
  // Handle localhost development (fallback)
