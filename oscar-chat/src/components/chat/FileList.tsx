...
export function FileList({ 
  ...
  const { addTab, closeTab, getTabByFile, isTabOpenByFile, switchToTab, activeTabId, openTabs } = useTabContext();
    ...
    if (file) {
      ...
      if (isTabOpenByFile(fileIdTyped)) {
        ...
        if (userOrg && userTeam) {
          ...
          console.log("Falling back to old format");
    ...
    try {
      if (deleteDialog.selectedIds) {
        ...
        Array.from(deleteDialog.selectedIds).forEach(id => {
          const tab = getTabByFile(id as Id<"files">);
          if (tab) {
            closeTab(tab.id);
          }
        });
          ...
          if (remainingTabs.length > 0) {
            ...
            if (remainingTab.fileId) {
              ...
              if (file && userOrg && userTeam) {
                ...
                router.push(`/chat?file=${remainingTab.fileId}`);
              }
          ...
          <div className="relative">
            <div className="h-[22px] flex items-center">
              <input
                ...
                className={cn(
                  "w-full h-[20px] px-5 text-[13px] bg-sidebar text-sidebar-foreground focus:outline-none",
            ...
            {createFileError && (
              <div className="absolute z-10 px-2 py-1 text-xs text-white bg-red-600 rounded shadow-lg max-w-64 break-words left-5 top-6">
                {createFileError}
              </div>
            )}
          </div>
